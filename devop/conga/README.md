# devop/conga — deploy config generator
...
> from one per-app *tenant* block. Build it with `mvn -pl devop/conga clean package`.
...
```bash
mvn -pl devop/conga conga:generate     # render config for the target box
# then let Ansible ship it:
cd ops/ansible && ansible-playbook -i inventory/hosts.yml playbooks/site.yml
```
...
publish this `devop/conga` roles + templates tree as a **versioned artifact**
