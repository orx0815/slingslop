...
async function buildCss(bundle) {
  ...
  // Relative path from the output file back to the source entry point,
  // used in the generated-file banner so a coding agent can find the sources.
  const banner = [
    `/*`,
    ` * !! DO NOT EDIT THIS FILE DIRECTLY !!`,
    ` *`,
    ` * This file is AUTO-GENERATED by the frontend build script.`,
    ` * All changes made here will be overwritten on the next build.`,
    ` *`,
    ` * Edit the source files instead:`,
    ` *   sling-apps/zengarden/zengarden.ui.apps/frontend/src/css/${name}/`,
    ` *`,
    ` * Then rebuild with:`,
    ` *   cd sling-apps/zengarden/zengarden.ui.apps/frontend && node scripts/bundle.js`,
    ` */`,
    ``,
  ].join('\n');

  const plain = banner + resolveImports(entry);
...
async function buildJs(bundle) {
  ...
  // plain (unminified) — inline source map for DevTools per-file navigation
  await esbuild.build({
    entryPoints: [entry],
    bundle:      true,
    format:      'iife',
  ...
  // minified (optionally with htmx prepended) — no source map
