...
(function (): void {
  ...
  function init(): void {
    ...
    document.body.addEventListener('htmx:after:swap', (event: Event) => {
      const htmxEvent = event as CustomEvent<{ ctx: { target: HTMLElement } }>;
      console.log('HTMX swap completed', htmxEvent.detail);

      // After successful upload, trigger asset list reload
      if (
        htmxEvent.detail.ctx.target &&
        htmxEvent.detail.ctx.target.classList.contains('dml-asset-grid')
      ) {
    ...
    document.body.addEventListener('htmx:after:request', (event: Event) => {
      const htmxEvent = event as CustomEvent<{
        ctx: { response: { status: number }; sourceElement: HTMLElement };
      }>;

      if (
        htmxEvent.detail.ctx.response.status < 400 &&
        htmxEvent.detail.ctx.sourceElement.classList.contains('dml-upload-form')
      ) {
        ...
        // Reset the form
        const form = htmxEvent.detail.ctx.sourceElement as HTMLFormElement;
        form.reset();
