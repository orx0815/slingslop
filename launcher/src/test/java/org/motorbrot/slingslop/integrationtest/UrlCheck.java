package org.motorbrot.slingslop.integrationtest;
...
class UrlCheck {
    ...
    static void runAll(CloseableHttpClient client, int retryCount, int msecBetweenRetries, Collection<UrlCheck> checks)
            throws Exception {
        for (UrlCheck check : checks) {
            ...
            if (lastFailure != null) {
                throw new RuntimeException(String.format(
                        "Starter not ready or path not found after %d tries (%d msec). Request to URL %s failed with message '%s'",
                        requestCount, System.currentTimeMillis() - start, check.getUrl(), lastFailure));
            }
        }
    }
}
