...
The Apache caching reverse proxy from `docker/webcache/` is reused. The per-app
vhosts are **generated by CONGA** and shipped by the `webcache` role: the whole
`devop/conga/target/configuration/{{ conga_env }}/{{ conga_node }}/webcache/`
...
> [`devop/conga`](../devop/conga/README.md) environment
> (`devop/conga/src/main/environments/<conga_env>.yaml`), rebuild
> (`mvn -pl devop/conga clean package`, or a full `mvn install` which also
...
environment (e.g. [`devop/conga/src/main/environments/prod-motorbrot.yaml`](../devop/conga/src/main/environments/prod-motorbrot.yaml))
...
- Traefik natively load-balances: its router service takes a **list** of
  `servers:` URLs. Today [`slingslop-edge/router.yml.hbs`](../devop/conga/src/main/templates/slingslop-edge/router.yml.hbs)
...
hosts, `RENDERER_URL` / `AUTHOR_URL` / the Traefik server URLs become **real
