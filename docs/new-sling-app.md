...
Ask ALF for each field in order. Once the **project name** is known, Smith derives a suggested value for every subsequent field and offers it as a ready-to-accept option — ALF can confirm it with a click or override it with a custom value.

For example, if ALF says `"Hello Sling World"`, Smith immediately offers `org.helloslingworld` as a groupId option, `hello-sling-world` as the artifactId, `/apps/hello-sling-world` as the apps path, etc.

| Field                           | Description                             | Smith's suggestion strategy                                                                                                             |
| ------------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| **Project name**                | Human-readable, e.g. "Nebula Dashboard" | Accept as-is                                                                                                                            |
...
| **Maven `artifactId` prefix**   | e.g. `nebula-dashboard`                 | Offer lower-case-hyphenated version of project name as quick option                                                                     |
| **Java package**                | e.g. `com.alfscompany.nebuladashboard`  | Offer `<groupId>.<artifactId-no-hyphens>` as quick option. This is where Sling Models, Servlets, and OSGi services will live            |
| **`sling:resourceType` prefix** | e.g. `nebula-dashboard`                 | Offer same as `artifactId` prefix as quick option. **Never** use `slingslop` — the new project must be independently nameable           |
| **Sample content JCR path**     | e.g. `/content/nebula-dashboard/sample-content` | Offer `/content/<projectname-lowercase>/sample-content` as quick option                                                                 |
| **Apps path**                   | e.g. `/apps/nebula-dashboard`           | Offer `/apps/<projectname-lowercase>` as quick option. This is where templates, components, CSS, and JS are deployed                    |
...
| **What does it do?** | Functional purpose, target audience, key content areas — drives page structure and component choices | Free text, or "surprise me" → Smith invents something with a clear purpose |
| **Tone & feel** | Subject matter for dummy text, personality of the writing — separate from visual mood | Free text (e.g. "serious B2B", "playful kids app", "dry academic"), or "surprise me" → Smith invents something thematic |
| **Favourite colours (2 max)** | OKLCH base hues | Any colour names/hex, or "surprise me" → Smith picks Matrix-green + a complementary |
| **Navigation style** | Layout structure | Top bar, left sidebar, hamburger menu, "surprise me" |
| **Zen-editable?** | Whether to include the tiptap/HTMX inline editing stack | Yes → copy `editable-component` / `editable-component-modal` supertypes, include tiptap in frontend. No → empty editor bundles, no tiptap dependency, simpler HTL |
| **Visual mood** | CSS direction | minimal, bold, editorial, technical, playful, "surprise me" |
| **Inspiration URLs (optional)** | Design reference | For design look, mood, or sample text |
...
Task  6 — Editor CSS partials (00-variables through 06-inline-editor)
...
Use these variables throughout (example values for a project named "Nebula Dashboard"):

```
PROJECT_NAME     = nebula-dashboard           # lower-case-hyphenated
ARTIFACT_PREFIX  = nebula-dashboard            # Maven artifactId prefix
GROUP_ID         = com.alfscompany             # Maven groupId
RT_PREFIX        = nebula-dashboard            # sling:resourceType prefix
CONTENT_ROOT     = /content/nebula-dashboard   # JCR content root
APPS_ROOT        = /apps/nebula-dashboard      # JCR apps root
JAVA_PACKAGE     = com.alfscompany.nebula      # Java package (derived from groupId + name)
```
...
<artifactId>slingslop.{PROJECT_NAME}.ui.apps</artifactId>
<packaging>content-package</packaging>
<name>Slingslop - {DISPLAY_NAME} UI Apps</name>
```
...
```xml
<artifactId>slingslop.{PROJECT_NAME}.core</artifactId>
...
```xml
<artifactId>slingslop.{PROJECT_NAME}.sample-content</artifactId>
<packaging>content-package</packaging>
```
...
```json
{
  "name": "slingslop-{PROJECT_NAME}-ui-apps",
...
### 5.3 basepage/html.html Template
...
### 5.6 Component HTL Pattern (View)
...
### 5.7 Component .content.xml Pattern
...
### 5.8 Sling Includes vs Sling Resources
...
**Smith should save tokens here. The bundle is scaffolding, not fleshed out.**
...
/content/{RT_PREFIX}/
  home/                                    ← sling:Folder
    .content.xml                           ← resourceType → pages/page
    _jcr_content/
      .content.xml                         ← resourceType → pages/homepage
      hero/                                ← or intro/banner
        .content.xml
      main/
        .content.xml
        text-block-1/
    ...
    styleguide/
      .content.xml                         ← resourceType → pages/page
      _jcr_content/
        .content.xml                       ← resourceType → pages/styleguide
        ... (component content nodes)
    content-page/
      .content.xml                         ← resourceType → pages/page
      _jcr_content/
        .content.xml                       ← resourceType → pages/contentpage
        hero/
          .content.xml
        main/
          .content.xml
...
See section 3.3 for the exact POM changes needed.
...
Include the explanation from section 5.1 of this skill — how the hue wheel works, where to change the two base hues, how all colours are derived.
